...
import {useLoginMutation} from '../../state/adminSlice' 
...
const Admin = () => {
    ...
    const handleSubmit = async(event) => {
        ...
        if (form.checkValidity() === false) {
            ...
            setValidated(true);
            login({email, password, isAdmin:true}).unwrap().then(res =>{
  ...
  return (
    <div className='adminlogin'>
        ...
        <div className="adminloginBody">
            ...
            {isLoading && 'loading...'}
            {!isLoading && <Button variant='warning' className='mt-3'  type="submit">Sign In</Button>}
            </Form>
        </div>
            </div>
        </div>
